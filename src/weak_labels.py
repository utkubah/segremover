...
def main() -> None:
    ...
    if FUNCTION_LABELS_PATH.exists():
        with FUNCTION_LABELS_PATH.open(encoding="utf-8") as f:
            for line in f:
                row = json.loads(line)
    ...
    if DISFLUENCY_LABELS_PATH.exists():
        with DISFLUENCY_LABELS_PATH.open(encoding="utf-8") as f:
            for line in f:
                row = json.loads(line)
